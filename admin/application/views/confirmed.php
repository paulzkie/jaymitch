...
if(confirm("Are you sure you want to cancel this reservation?")==true){
        ...
        $.ajax({
            type: "POST",
            url: base_url+"confirmed/cancel",
            data: {reserveid: $(this).data("reserveid"), email: $(this).data("email")},
            success: function(data){
              alert("Successfully cancelled.");
              $("#datacontainer").load(base_url+"index.php/confirmed #datalist");
              $("#add_err").hide();
            },
            beforeSend: function(){
            ...
            <table  id="datalist" class="table" ui-jq="footable" ui-options='{
              ...
              <?php foreach($userdata as $data){ ?>
                <tr data-expanded="true">
                  ...
                  <td><img src="<?php echo base_url()?>assets/paymentImages/<?php echo $data->image?>" width="144" height="144" d="myImg" 
                   style="cursor:pointer" onclick="showImage(this.src, '<?php echo base_url()?>assets/paymentImages/<?php echo $data->image?>');"></td>
                  <td>
                  <a href="view?reserveid=<?php echo $data->reserveid?>&code=<?php echo $data->transactioncode?>"><button class="btn-primary update" type="submit">View</button></a>
                  <button class="btn-warning cancel" id="cancel" data-reserveid="<?php echo $data->reserveid?>" data-email="<?php echo $data->email?>" type="submit">Cancel</button>    
                  </td>
                </tr> 
                 <?php } ?>
              </tbody>
