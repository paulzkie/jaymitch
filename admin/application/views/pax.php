...
$(document).on('click', '#add', function(e){
    ...
    $.ajax({
        type: "POST",
        url: base_url+"index.php/pax/insert",
        data: $("#paxform").serialize(),
        success: function(data){
            var message = JSON.parse(data);
            if(message.stat==1){
                alert(message.msg);
                $("input").val("");
                $("#datacontainer").load(base_url+"index.php/pax #datalist");     
                $("#add_err").hide(); 
            }else{
                alert(message.msg);
                $("#add_err").hide(); 
...
$(document).on('click', '.delete', function(e){
    ...
    if(confirm("Are you sure you want to deleted this?")==true){
            $.ajax({
                type: "POST",
                url: base_url+"index.php/pax/delete",
                data: {paxid: $(this).data("paxid"),},
                success: function(data){
                    var message = JSON.parse(data);
                    if(message.stat==1){
                        alert(message.msg);
                ...
                <div class="col-lg-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <div class="form">
                                <form class="cmxform form-horizontal " id="paxform" method="post">
                                    <div class="form-group ">
                                        <label for="time" class="control-label col-lg-3">Pax:</label>
                                        <div class="col-lg-6">
      ...
      <table  id="datalist" class="table" ui-jq="footable" ui-options='{
        ...
        <?php foreach($userdata as $data){ ?>
          <tr data-expanded="true">
            ...
            data-paxid="<?php echo $data->paxid?>"
            data-pax="<?php echo $data->pax?>">Update</button><td>
            <button class="btn-warning delete" data-paxid="<?php echo $data->paxid?>" type="submit">Delete</button>    
            </td></td>
          </tr> 
           <?php } ?>
        </tbody>
